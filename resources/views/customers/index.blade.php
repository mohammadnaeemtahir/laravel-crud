@extends('layouts.app', ['title' => 'Register Customer'])

@section('content')
<div class="container">
    <div class="card p-4 shadow">
        <h3 class="text-center text-wrap fw-bold">{{'Register Customer'}}</h3>
        <form action="{{route('customer.store')}}" method="post">
            ...
            <div class="row">
                ...
                <div class="mb-3 col-md-6">
                    ...
                    <input type="email" name="email" class="form-control" placeholder="i.e. example@email.com" value="{{old('email')}}">
                    <span class="text-danger">
                        @error('email')
                        {{$message}}
                        @enderror
                ...
                <div class="mb-3 col-md-6">
                    ...
                    <input type="tel" name="phone_number" class="form-control" placeholder="Enter your valid phone number" value="{{old('phone_number')}}">
                    <span class="text-danger">
                        @error('phone_number')
                        {{$message}}
                        @enderror
                ...
                <div class="mb-3 col-md-6">
                    ...
                    <input type="number" name="age" class="form-control" placeholder="Enter your age (number)" value="{{old('age')}}">
                    <span class="text-danger">
                        @error('age')
                        {{$message}}
                        @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea name="address" class="form-control" placeholder="Enter your current address" value="{{old('address')}}"></textarea>
                    <span class="text-danger">
                        @error('address')
                        {{$message}}
                        @enderror
                ...
                <button type="submit" class="btn btn-primary">{{__('Register Now')}}</button>
            </div>
        </form>
    </div>
</div>
